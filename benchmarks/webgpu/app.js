import {Cloth} from './physics.js?v=7';
...
function failure(error){running=false;measuring=null;errors.push(String(error));status(String(error));$('run').textContent='Run simulation';console.error(error);}
function controls(disabled){for(const id of ['garment','run','reset','width','widen','wind','measure','substeps','stretch','bend','body-contact','self-contact','continue'])$(id).disabled=disabled;}
function settings(){
...
async function load(){
 loading=true;running=false;controls(true);$('run').textContent='Run simulation';$('download').disabled=true;
...
function reset(){running=false;cloth.reset();renderer.dirty=true;$('run').textContent='Run simulation';$('elapsed').textContent='0.0 s';status('Original panels restored.');lastResult=null;$('download').disabled=true;}
function summarize(positions,initial){
 const s=cloth.scene,stretch=[],seams=[];let maxMove=0,finite=true;
 ...
 return {finite,rms_velocity_m_s:cloth.rmsVelocity,displacement_max_m:maxMove,edge_stretch_p95:percentile(stretch,.95),edge_stretch_max:Math.max(...stretch),seam_gap_p95_mm:percentile(seams,.95),seam_gap_max_mm:Math.max(...seams),bounds_m:[0,1,2].map(a=>[Math.min(...positions.map(p=>p[a])),Math.max(...positions.map(p=>p[a]))])};
}
async function finish(){
 ...
 lastResult={engine:'Browser WebGPU small-step XPBD',scene:cloth.scene.name,start_frame:m.startFrame,frames:m.frames,simulated_seconds:m.frames/60,wall_s:wall,updates_per_second:m.frames/wall,
  ...
  settings:{...cloth.settings},adapter:adapter.info?{vendor:adapter.info.vendor,architecture:adapter.info.architecture,device:adapter.info.device,description:adapter.info.description}:null,
  user_agent:navigator.userAgent,initialization:m.startFrame?'Continue browser-generated drape at fixed topology':'Original unsewn panels raised 6 cm, sew for 0.8 s before gravity; no precomputed drape',quality:summarize(lastPositions,m.initial),
  timestamp_query:!!queries,errors:[...errors],frame_ms:m.frameMs,gpu_physics_ms:m.gpu,gpu_render_ms:m.render,
  limitations:['Distance-based stretch and approximate bending, uncalibrated fabric',
 ...
 controls(false);$('download').disabled=false;$('run').textContent='Run simulation';status('Measurement finished. Inspect the drape, then save the result.');
}
  ...
  try{
   ...
   if(simulated){
    ...
    if(measuring){
     ...
     status(`Measuring ${measuring.frames} / 300 frames. All physics runs in this browser.`);
     if(measuring.frames===300){measuring.ended=performance.now();await finish();}
    }
   }
...
$('reset').onclick=reset;
for(const id of ['width','wind','substeps','stretch','bend','body-contact','self-contact'])$(id).oninput=settings;
...
$('measure').onclick=async()=>{running=false;controls(true);if(!$('continue').checked)cloth.reset();settings();$('download').disabled=true;displaySamples=[];lastUpdateTime=0;const initial=await cloth.readPositions();measuring={frames:0,startFrame:cloth.frame,initial,started:performance.now(),frameMs:[],gpu:[],render:[]};running=true;};
