...
try {
 ...
 const inspect=async()=>{
  ...
  status.textContent=JSON.stringify({frame:cloth.frame,msPerFrame:ms,bodyYaw:cloth.motion.yaw,motionReport,creases:scene.crease_constraints,seamMax:Math.max(...seams.map(gap)),kernelChecks:cloth.kernelChecks,panels},null,2);
 };
 const run=async(n)=>{
  for(const b of document.querySelectorAll('button'))b.disabled=true;
 ...
 motionButton.onclick=async()=>{
  ...
  for(let i=0;i<600;i++){
   ...
   const e=device.createCommandEncoder();cloth.encode(e);renderer.render(e);device.queue.submit([e.finish()]);await device.queue.onSubmittedWorkDone();
   if(i%30===0){
