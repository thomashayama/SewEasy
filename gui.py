...
@app.get('/health')
def health():
    """Lightweight probe for container/cloud healthchecks (does not create GUI state)"""
    return {'status': 'ok'}
...
async def index(client: Client, request: Request):
    ...
    try:
        ...
        if request.query_params.get('measurements') == '1':
            gui_st.ui_measurements_dialog.open()
